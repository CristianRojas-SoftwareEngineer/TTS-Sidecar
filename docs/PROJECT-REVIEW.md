...
|---|---|
| **Severidad** | Mayor |
| **Evidencia** | `cli.py:166-185` (`cmd_voice_add` instancia `ChatterboxEngine(...)`) → `engine.py:216` (`__init__` llama `_download_model`). El gate `_require_model_cached` (utilizado en `cmd_speak` y `cmd_daemon` con `start`) **no se invoca** desde `cmd_voice_add`. |
...
| **Propuesta** | Añadir `_require_model_cached()` al inicio de `cmd_voice_add` (`cli.py:169-185`) para abortar temprano si el modelo no está cacheado. Eliminar la inicialización en caliente del motor de inferencia en `voice add` diferiendo la computación de `conditionals.pt` a la primera síntesis (`speak`) con esa voz; `voice add` solo validará y copiará los audios WAV. |
...
| **Propuesta** | Implementar una captura de excepciones centralizada de `KeyboardInterrupt` en la función `main()` de `cli.py` (alrededor de `args.func(args)`) que imprima un mensaje a stderr y termine con `sys.exit(130)` (código estándar para procesos abortados por SIGINT). |
...
|---|---|
| **Severidad** | Mayor |
| **Evidencia** | `timing.log()` (`timing.py:11-21`) y `timed_command` (`timing.py:33,37,41`) usan `print()` sin `file=sys.stderr`. Todo el progreso por etapas de `speak`/`voice add` va a **stdout**. USAGE §«Experiencia unificada» (`USAGE.md:479-481`) repite el mismo contrato incorrecto. |
...
| **Tradeoffs y Justificación** | La opción elegida (Diagnósticos a stderr) respeta las directrices estándar UNIX y simplifica la vida del integrador. Alternativas evaluadas: (A) Añadir flags `--quiet` / `--verbose`: los integradores que deseen progreso y datos simultáneamente seguirán en conflicto. (B) Usar prefijos en stdout (ej: `[LOG] ...`): obliga a implementar parsers de cadenas en el consumidor programático. Justificación: Redirigir a stderr es el estándar estándar de la industria y la solución más limpia para integradores. |

#### R-06 — Códigos de salida sin granularidad

| | |
|---|---|
| **Severidad** | Mayor |
| **Evidencia** | `grep -n 'sys.exit' src/chatterbox_tts/cli.py` devuelve **22 llamadas a `sys.exit(1)`** (líneas 102, 112, 160, 163, 185, 202, 205, 218, 221, 247, 250, 261, 365, 378, 429, 471, 498, 531, 538, 545), todas con código 1. No hay distinción entre «modelo no cacheado», «argumento inválido», «fallo de síntesis» o «voz no encontrada». El contrato (`cli.py:9`) solo promete 0/≠0. |
...
| **Propuesta** | Declarar el flag `--port` en todos los subparsers del comando `daemon` (`start`, `stop`, `restart`, `status`) y usar una variable de entorno `TTS_SIDECAR_DAEMON_PORT` como canal de comunicación implícito de fallback para `DaemonIPCClient`. |
...
| **Propuesta** | Refactorizar `hub_cache_path()` in `model_cache.py` para consultar en orden de precedencia estándar de Hugging Face: `HF_HUB_CACHE`, `HF_HOME`/hub, y finalmente `~/.cache/huggingface/hub` como fallback. |
...
La cobertura es real y bien dirigida en zonas críticas: validación de rutas de voces, seguridad del daemon, resolución de caché, semántica setup-vs-doctor, construcción del instalador Windows. El CI corre `compileall src/` como red de sintaxis (`config.yml:28-29`). **Conteo verificado: 148 tests** (`pytest --collect-only` → `148 tests collected in 0.50s`).
...
| **Evidencia** | `docs/GOAL.md:140, 156` dice «95/95» y «95 tests». `CLAUDE.md` dice «95 tests». El enunciado original de esta auditoría decía «139», lo cual es correcto. **Conteo real verificado en este repo: `pytest tests/ --collect-only -q` → `139 tests collected`**. |
...
| R-06 | 2 | Mayor | Códigos de salida sin granularidad (22 sys.exit(1)) |
...
*Auditoría realizada el 2026-07-02. Revisión adversarial-constructiva aplicada el mismo día. Código base: commit `a0a77cc`. Conteo de tests verificado: 139.*