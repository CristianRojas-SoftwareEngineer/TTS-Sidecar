...
class TestCmdVoices:
    ...
    def test_cmd_voices_lists_voices(self, mock_engine_cls, capsys):
        from chatterbox_tts.cli import cmd_voices

        mock_engine = MagicMock()
        mock_engine.list_voices.return_value = ["crist", "testcli"]
    ...
    @patch("chatterbox_tts.engine.ChatterboxEngine")
    def test_cmd_voices_empty(self, mock_engine_cls, capsys):
        from chatterbox_tts.cli import cmd_voices
...
class TestCmdVoiceRemove:
    @patch("chatterbox_tts.engine.ChatterboxEngine")
    def test_cmd_voice_remove_success(self, mock_engine_cls, capsys):
        from chatterbox_tts.cli import cmd_voice_remove

        mock_engine = MagicMock()
        mock_engine.remove_voice.return_value = True
        mock_engine_cls.return_value = mock_engine
    ...
    @patch("chatterbox_tts.engine.ChatterboxEngine")
    def test_cmd_voice_remove_not_found(self, mock_engine_cls, capsys):
        from chatterbox_tts.cli import cmd_voice_remove

        mock_engine = MagicMock()
        mock_engine.remove_voice.return_value = False
        mock_engine_cls.return_value = mock_engine
